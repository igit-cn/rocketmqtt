...
type Config struct {
	...
	WithBrokerId   string    `json:"withBrokerId"`
	...
	DeliverMap []DeliverTarget `json:"deliverMap"`
...
type DeliverTarget struct {
	...
	Tag       string    `json:"tag"`
	NameSplit *[]string `json:"-"`
}

type Plugins struct {
	Auth     auth.Auth `json:"-"`
	AuthName string    `json:"auth"`
	//	Bridge   bridge.BridgeMQ `json:"bridge"`
...
func ConfigureConfig(args []string) (*Config, error) {
	...
	fs.Bool("D", false, "enable Debug logging.")

	if len(os.Args) == 1 {
		configFile = "conf/liumqtt.json"
	}
	...
	if config.Debug {
		log = logger.Debug().Named("conf")
	} else {
		log = logger.Prod().Named("conf")
	}
...
func LoadConfig(filename string) (*Config, error) {
	...
	config.Plugin.Auth = auth.NewAuth(config.Plugin.AuthName)

	for i, deliver := range config.DeliverMap {
		s := strings.Split(deliver.Match, "/")
		deliver.NameSplit = &s
		config.DeliverMap[i] = deliver
	}
